...
14. **G2 validates against a version this project resolves for itself, not a customer's own pinned version — fine today, wrong the moment the generation pivot is real for someone else.** `schema_lint.py`'s schema fetch (reused from `schema_watch.py`) reads the version constraint out of `synthesizer.py`'s own `required_providers` template (`_version_constraint()`, `schema_watch.py:63-73`) and resolves it live against the public registry via a real `terraform init` — there is no "customer's environment" concept anywhere in this path, because none exists yet: single operator, one fixed constraint, one resolution. That's a correct, disclosed simplification for a solo-operator tool. It becomes load-bearing wrong the moment the generation pivot's actual premise — version-adaptive authoring **for the customer's pinned version** — is real: if a customer's own lockfile pins `hashicorp/aws` 5.31 and MinusOps' `>= 5.0` constraint resolves to 6.2 on the registry that day, G2 validates newly-authored HCL against the wrong schema entirely — content that passes G2 clean could fail the customer's own `terraform plan`, or the reverse. **Not fixed, not scoped now** — recorded as the explicit revisit condition for whenever a real multi-tenant/real-customer-environment concept exists, same "resolution pending a positioning decision" shape as item 1: a single-operator-tool-vs-product divergence point, not a bug to patch today.
