...
- [`tests/test_lineage_graph.py`](./test_lineage_graph.py): The five medallion hops for a full
  ...
  which is which.
- [`tests/test_blueprint_corpus.py`](./test_blueprint_corpus.py): The reference-architecture corpus in `core/architecture/blueprint_data/`, and what it is allowed to say. Its load-bearing assertion is that no service a tenth of the corpus names classifies as `other` -- the layer with no meaning, which lands in the diagram footer. That is what caught `aws_vpc` (10 of 35 sources) matching neither `_vpc` nor `vpc_`, both of which need a delimiter a bare type does not have. Also asserts the corpus stores counts and never page text, and that every role the rules can return has a layer.
