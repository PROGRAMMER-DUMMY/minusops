...
| **14** | **Multi-environment proving & lifecycle testing** | **"How should MinusOps test and validate the pipeline across your environments (Dev, Test, UAT, Prod)?"** MinusOps is the complete lifecycle control plane across all tiers. Ask how verification should run: (A) **Automated Synthetic Proving in Test/Dev** (`minusctl seed --execute`: seeds synthetic Bronze data, runs PySpark/Airflow, verifies Great Expectations DQ, queries Athena Gold, and records a step-by-step verification evidence report before promoting through UAT to Prod); (B) **Local G9 Ephemeral Simulation** (`minusctl demo`: validates flows and emits trace reports with zero AWS spend); or (C) **Gated Multi-Stage Promotion** (runs plan-gate verification with stage-specific `tfvars` at each Dev $\to$ Test $\to$ UAT $\to$ Prod promotion gate). | `core/reporting/seed.py` · `core/generation/demo.py` · G9 Ephemeral Gate · `plan_gate.py` |
