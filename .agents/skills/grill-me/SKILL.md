...
| **11** | **Control plane hosting** | **How will they drive MinusOps?** Three answers, and they need different artifacts: a **local CLI** on engineer laptops (ambient AWS SSO), a **CI/CD gate** (GitHub Actions or Jenkins, OIDC `AssumeRoleWithWebIdentity`, zero static keys), or a **24/7 web console** in-cluster (EKS with IRSA, or ECS Fargate with a task role). Then: is the console reachable from anything but localhost, and if so is it behind `MINUS_DASH_TOKEN` **and** an internal-only ALB? The console renders live account identity and spend -- an internet-facing scheme on that ingress is the whole exposure. | `Dockerfile` - `deploy/k8s/` - `core/generation/cicd.py` |
| **12** | **Semantic layer & AI readiness** | **Will this platform serve self-service BI or AI/LLM agents (Text-to-SQL / Chat)?** Where should business metric logic live: **code-native** (dbt Semantic Layer / MetricFlow YAML in Git), **headless universal** (Cube.js hosted on EKS with REST/GraphQL/SQL APIs), or **catalog-native** (Glue Catalog Views / Athena)? Without a semantic layer, LLMs hallucinate joins and miss filters; with one, they query governed metrics. | `dbt-semantic-layer` · `cube-semantic-layer` · `query-athena` |
| **13** | **Data governance & access control** | **Are Row-Level Security (RLS) or PII Column Masking required on Gold/Serving?** Lake Formation Tag-Based Access Control (LF-TBAC) provides centralized column masking and row filters for Athena/EMR; Redshift/Snowflake native grants handle warehouse queries. Asking upfront determines whether AWS Lake Formation or simple IAM policies are provisioned. | `governance-lakeformation` · `security-iam-scoped` |
| **14** | **Multi-environment proving & lifecycle testing** | **"How should MinusOps test and validate the pipeline across your environments (Dev, Test, UAT, Prod)?"** MinusOps is the complete lifecycle control plane across all tiers. Ask how verification should run: (A) **Automated Synthetic Proving in Test/Dev** (`minusctl prove --execute`: the 5-hop harness -- seeds synthetic Bronze data, runs PySpark/Airflow, evaluates Great Expectations DQ, verifies malformed rows reach quarantine with none lost, queries Athena Gold, and writes a tamper-evident `proving_report.json` before promoting through UAT to Prod); (B) **Local G9 Ephemeral Simulation** (`minusctl demo`: validates flows and emits trace reports with zero AWS spend); or (C) **Gated Multi-Stage Promotion** (runs plan-gate verification with stage-specific `tfvars` at each Dev $\to$ Test $\to$ UAT $\to$ Prod promotion gate). | `core/reporting/seed.py` · `core/generation/demo.py` · G9 Ephemeral Gate · `plan_gate.py` |
