...
| **14** | **Pre-flight proving & sandbox testing** | **"Would you like an autonomous agent to test and prove this architecture first with synthetic data before production handover?"** Production teams don't hand over empty infrastructure. Ask: (A) **Full Ephemeral Proving** (`minusctl seed --execute` in sandbox: seeds Bronze, runs Glue/Airflow, validates Great Expectations DQ, queries Gold via Athena, and outputs a step-by-step verification report); (B) **Offline Simulation** (`minusctl demo`: generates synthetic DAG flows, connection configs, and trace reports without AWS mutation); or (C) **Direct Plan-Gate Review**. | `core/reporting/seed.py` · `core/generation/demo.py` · G9 Ephemeral Gate |
