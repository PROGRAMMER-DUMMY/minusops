...
def _resource_count_amount(plan, entry, A):
    """Generic 'amount = resource count (x an optional constant)' dispatcher, for services
    whose usage IS just how many of a resource exist -- no per-service Python branch needed
    to add the next one of these (e.g. Secrets Manager secrets, Route53 hosted zones). Add
    amount_model/amount_count_prefix/amount_constant to the entry in aws_resource_map.json
    instead. Returns None if no matching resources exist (never a fabricated zero)."""
    prefix = entry.get("amount_count_prefix", entry["prefix"])
    count = len(plan_inventory_addresses(plan, prefix))
    if count <= 0:
        return None
    constant_key = entry.get("amount_constant")
    multiplier = A[constant_key] if constant_key else 1
    return round(count * multiplier, 2)
...
def _amount_for(service_code, inputs, A, plan):
    ...
    """
    entry = pricing_catalog.entry_for_service_code(service_code)
    if entry and entry.get("amount_model") == "resource_count":
        return _resource_count_amount(plan, entry, A)
    ...
    # AmazonMWAA, AmazonCloudWatch (metric alarms), and awskms (KMS keys) are handled above by
    # the generic resource_count dispatcher, driven by aws_resource_map.json's amount_model.
    ...
    # AWSLambda / AmazonDynamoDB / AWSStepFunctions (no module wires a usage driver yet)
    # intentionally fall through to None — REVIEW_REQUIRED via a reviewed usage profile.
    return None
