...
- **Wired into the gate (PRD v6 FR-07/AC-05)**: [`plan_gate._drift_for_plan(plan_json, with_telemetry)`](./plan_gate.py) is the only caller that supplies a lookup, and only when [`_telemetry_requested`](./plan_gate.py) says so -- `--with-telemetry` or `MINUS_TELEMETRY=1`. Off by default because the common case is an offline plan on a laptop with no credentials, where reaching for CloudTrail is slower for no answer.
- **Output shape**: [`format_result`](./cloud_drift.py) groups by RESOURCE, not by finding type. The declared value, the live value, who changed it and what failed beforehand are one story about one resource; split across three lists they are four facts a reviewer has to reassemble. Each drifted row carries `declared` and `live` as VALUES (`worker_type = G.1X` vs `G.2X`), not just attribute names -- the names identify the event, the values are the decision. The clean case is still one line, because it is what every passing plan prints.
- **[`plan_gate.gate_status(dir_)`](./plan_gate.py)** (PRD v6 FR-04) reports recorded gate state from disk only. It deliberately does not re-hash the plan: that shells out to `terraform show` on every call, needs an initialised directory, and turns a status check into a slow credential-shaped operation. An approval for a *different* hash is reported as not approved -- that is the whole point of the hash binding, and softening it in the one place an operator looks before `apply` would undo it.
- **[`plan_gate._reject_if_not_asserted_role(posture, role_arn)`](./plan_gate.py)** verifies `--role-arn`: an assertion that the ambient session IS that deploy role. The gate never assumes a role and never handles credentials, so this checks the posture it already reads and fails closed on an unreadable identity. **There is no `--mfa-arn`**: `sts get-caller-identity` returns Account, Arn and UserId and carries no MFA claim, so the gate cannot verify one. MFA is enforced by the role's trust policy (`aws:MultiFactorAuthPresent`) at AssumeRole time, upstream of this process. A flag accepted and never checked would be an unverified assertion sitting in an audit record.
- **[`RECOMMENDATION`](./cloud_drift.py) needs both signals**: evidence of why, AND a plan that would undo it. Without evidence "do not revert" is advice we cannot support; without a revert there is nothing to not-revert.
- **Wired into the gate (PRD v6 FR-07/AC-05)**: [`plan_gate._drift_for_plan(plan_json, with_telemetry)`](./plan_gate.py) is the only caller that supplies a lookup, and only when [`_telemetry_requested`](./plan_gate.py) says so -- `--with-telemetry` or `MINUS_TELEMETRY=1`. Off by default because the common case is an offline plan on a laptop with no credentials, where reaching for CloudTrail is slower for no answer.
- **Output shape**: [`format_result`](./cloud_drift.py) groups by RESOURCE, not by finding type. The declared value, the live value, who changed it and what failed beforehand are one story about one resource; split across three lists they are four facts a reviewer has to reassemble. Each drifted row carries `declared` and `live` as VALUES (`worker_type = G.1X` vs `G.2X`), not just attribute names -- the names identify the event, the values are the decision. The clean case is still one line, because it is what every passing plan prints.
- **[`RECOMMENDATION`](./cloud_drift.py) needs both signals**: evidence of why, AND a plan that would undo it. Without evidence "do not revert" is advice we cannot support; without a revert there is nothing to not-revert.
