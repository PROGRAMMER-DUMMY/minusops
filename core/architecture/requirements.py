...
_BUDGET_RE = re.compile(r"\$\s?(\d+(?:,\d{3})*(?:\.\d+)?)")
...
def parse_budget_usd(data):
    ...
    take the SMALLEST (the guardrail should trip earlier, not later, when ambiguous). The
    source text is returned so the assumption stays auditable. (0, "") when nothing
    parseable -- never a guess. Canonical here; synthesizer consumes it the same way it
    consumes parse_daily_gb.
    """
    text = str((data or {}).get("non_functional", {}).get("budget") or "")
    amounts = [float(m.replace(",", "")) for m in _BUDGET_RE.findall(text)]
    return (min(amounts), text.strip()) if amounts else (0, "")
